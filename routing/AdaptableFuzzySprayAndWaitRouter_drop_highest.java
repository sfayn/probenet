 ...
 *
 */
public class AdaptableFuzzySprayAndWaitRouter_drop_highest extends EnergyAwareRouter {
	...
	public AdaptableFuzzySprayAndWaitRouter_drop_highest(Settings s) throws IOException {
		super(s);
	 ...
	 */
	protected AdaptableFuzzySprayAndWaitRouter_drop_highest(AdaptableFuzzySprayAndWaitRouter_drop_highest r) {
		super(r);
	...
	protected Message getOldestMessage(boolean excludeMsgBeingSent) {
		...
		for (Message m : messages) {
			...
			if (highest_message == null ) {
				highest_message = m;
				highest_priority=f.getPriority(highest_message);
			}
			else if (highest_priority < f.getPriority(m)) {
				highest_message = m;
				highest_priority=f.getPriority(highest_message);
			}
		}
	...
	public void changedConnection(Connection con) {
		if (con.isUp()) { // new connection
			if (con.isInitiator(getHost())) {
				...
				assert mRouter instanceof AdaptableFuzzySprayAndWaitRouter_drop_highest : "FuzzySprayAndWaitRouter only works "+
				" with other routers of same type";
				AdaptableFuzzySprayAndWaitRouter_drop_highest otherRouter = (AdaptableFuzzySprayAndWaitRouter_drop_highest)mRouter;
	...
	@Override
	public AdaptableFuzzySprayAndWaitRouter_drop_highest replicate() {
		return new AdaptableFuzzySprayAndWaitRouter_drop_highest(this);
	}
}
