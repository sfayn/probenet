...
FOR /F "delims=" %%f IN (%1_file) DO FOR %%m in (latency,av_latency) do gawk -f latency.awk metric="%%m" min=%MIN_VALUE% %%f >> ..\values\%1_%%m_1
FOR %%m in (latency,av_latency) do gawk -f sort.awk  ..\values\%1_%%m_1 > ..\values\%1_%%m
FOR %%m in (latency,av_latency) do erase  ..\values\%1_%%m_1,..\values\%1_%%m
erase temp_files, %1_file,temp,min
