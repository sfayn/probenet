...
FOR %%r in (SW,AFS, AFSnW, AFSnA) do call per_router.bat %%r
erase min,temp
FOR %%r in (SW,AFS, AFSnW, AFSnA) do FOR /F "delims=" %%f IN (%%r_file) do gawk -f max_cum_prob.awk %%f >> temp
...
FOR %%y in (delivery_prob,overhead_ratio, average_energy,dead_nodes,latency,av_latency) do gnuplot -e "set term png size 1280, 800;set output '../graphs/%1_%%y.png'  ; set logscale x;set ylabel '%%y';set xlabel '%X_label%';plot '../values/AFSnW_%%y' using 1:2 title 'AFSnW' with lines, '../values/AFS_%%y' using 1:2 title 'AFS' with lines, '../values/SW_%%y' using 1:2 title 'SW' with lines, '../values/AFSnA_%%y' using 1:2 title 'AFS_nA' with lines";
