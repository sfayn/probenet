...
FOR %%r in (SW,AFS, AFSnW) do FOR %%m in (latency,av_latency) do gawk -f merge_equal_results.awk  ..\values\%%r_%%m_1 > ..\values\%%r_%%m
FOR %%r in (SW,AFS, AFSnW) do FOR %%m in (latency,av_latency) do erase  ..\values\%%r_%%m_1 
REM , %%r_file
erase min,temp
