...
FOR /F "delims=" %%f IN (%1_file) DO FOR %%m in (delivery_prob,overhead_ratio, average_energy,dead_nodes) do gawk -f average_2var.awk metric="%%m" %%f >> ..\values\%1_%%m
REM FOR %%m in (delivery_prob,overhead_ratio, average_energy,dead_nodes) do gawk -f sort_2var.awk  ..\values\%1_%%m_1 > ..\values\%1_%%m
